...
exports.notifyFO = exports.sendFoBulkNotification = exports.sendVendorBulkNotification = exports.sendVendorNotification = exports.sendOTP = void 0;
...
const resolveRecipientEmail = (actualEmail) => {
...
const normalizeRole = (role) => safeValue(role).toUpperCase().replace(/[\s-]+/g, '_');
const isVendorCoordinatorRole = (role) => {
...
const isStaleLock = (lockValue) => {
    ...
    const lockRecord = lockValue;
    const lockedAt = lockRecord.lockedAt;
    if (!(lockedAt instanceof firebase_admin_1.default.firestore.Timestamp)) {
        return false;
    }
...
const requireVendorCoordinator = async (req) => {
...
const resolveAssignedFo = async (requestData) => {
    const assignedFoId = safeValue(requestData.assignedFoId) ||
...
exports.sendVendorBulkNotification = (0, https_1.onRequest)({ invoker: 'public' }, (req, res) => {
    return corsHandler(req, res, async () => {
        try {
            ...
            const { vendorName, rows, requestIds } = req.body;
            ...
            const payloadRowMap = new Map();
            const payloadRequestFallbackMap = new Map();
            rows.forEach((row) => {
            ...
            if (requestIds && requestIds.length > 0) {
                ...
                sanitizedRows = requestDocs
                    ...
                    .flatMap((doc) => {
                    const data = doc.data();
                    const baseDate = safeValue(data?.createdAt?.toDate?.()?.toISOString?.() || data?.createdAt);
                    const buildRow = (vehicle) => {
                        const hasVehiclePaymentSignals = vehicle && (vehicle.paymentApproved !== undefined || vehicle.paymentRejected !== undefined);
                        if (vehicle) {
                        ...
                        const payloadOverlay = payloadRowMap.get(`${doc.id}::${vehicleNumber}`) ||
                        ...
                        const ltpocDetails = Array.isArray(data?.ltpocDetails)
                            ? data.ltpocDetails.find((ltpoc) => ltpoc?.vehicleNumber === vehicleNumber)
                            : null;
                        const ltpocFromVehicle = vehicle?.ltpocDetails?.[0] || vehicle;
                        const resolvedServiceType = safeValue(payloadOverlay?.serviceType) ||
                            safeValue(vehicle?.serviceType) ||
                            safeValue(data?.serviceType);
                        const resolvedVehicleNumber = safeValue(payloadOverlay?.vehicleNumber) ||
                            vehicleNumber;
                        return {
                            requestId: doc.id,
                            ...
                            vehicleAvailabilityLocation: safeValue(payloadOverlay?.vehicleAvailabilityLocation) ||
                                safeValue(vehicle?.vehicleAvailabilityLocation || data?.vehicleAvailabilityLocation),
                            vehicleAvailableTime: safeValue(payloadOverlay?.vehicleAvailableTime) ||
                                safeValue(vehicle?.vehicleAvailableTime || data?.vehicleAvailableTime),
                            ltpocName: safeValue(payloadOverlay?.ltpocName) ||
                                safeValue(ltpocFromVehicle?.ltpocName || ltpocDetails?.ltpocName),
                            ltpocPhone: safeValue(payloadOverlay?.ltpocPhone) ||
                                safeValue(ltpocFromVehicle?.ltpocPhone || ltpocDetails?.ltpocPhone),
                            ltpocEmail: safeValue(payloadOverlay?.ltpocEmail) ||
                    ...
                    if (vehicles.length > 0) {
                        return vehicles.map((vehicle) => buildRow(vehicle)).filter(Boolean);
                    }
                ...
                sanitizedRows = rows
                    ...
                    ltpocEmail: safeValue(row.ltpocEmail),
            ...
            if (sentRequestIds.length > 0) {
                ...
                sentRequestIds.forEach((requestId) => {
                    postSendBatch.update(firestore.collection('requests').doc(requestId), {
                        vendorBulkMailSentAt: firebase_admin_1.default.firestore.FieldValue.serverTimestamp(),
...
exports.sendFoBulkNotification = (0, https_1.onRequest)({ invoker: 'public' }, (req, res) => {
    return corsHandler(req, res, async () => {
        try {
            const callerUid = await requireVendorCoordinator(req);
            const { requestIds, rows } = req.body;
            ...
            const lockPayload = {
                operationId,
                lockedAt: firebase_admin_1.default.firestore.FieldValue.serverTimestamp(),
                lockedBy: callerUid,
            };
            const lockedRequestData = new Map();
            await firestore.runTransaction(async (tx) => {
                ...
                snapshots.forEach((snapshot) => {
                    ...
                    const alreadyNotified = toBooleanFlag(requestData.foNotified) ||
                        Boolean(requestData.foBulkNotifiedAt) ||
                        Boolean(requestData.foBulkNotificationSentAt);
                    const lockedByAnotherActiveOp = Boolean(requestData.foNotificationLock && !isStaleLock(requestData.foNotificationLock));
            ...
            if (groupedByFo.size === 0) {
                ...
                lockedRequestData.forEach((_value, requestId) => {
                    unlockBatch.update(firestore.collection('requests').doc(requestId), {
                        foNotificationLock: firebase_admin_1.default.firestore.FieldValue.delete(),
                    });
                });
            ...
            `,
                    ...
                    foGroup.requestIds.forEach((requestId) => {
                        successBatch.update(firestore.collection('requests').doc(requestId), {
                            foNotified: true,
                            foNotifiedAt: firebase_admin_1.default.firestore.FieldValue.serverTimestamp(),
                            notifiedAt: firebase_admin_1.default.firestore.FieldValue.serverTimestamp(),
...
exports.notifyFO = (0, https_1.onRequest)((req, res) => {
    return corsHandler(req, res, async () => {
        try {
            const payload = req.method === 'GET' ? req.query : req.body;
            const getParam = (value) => (Array.isArray(value) ? value[0] : value);
            const payloadRecord = payload;
            const vehicleId = getParam(payloadRecord.vehicleId);
            const installationDateParam = getParam(payloadRecord.installationDate);
            const requestId = getParam(payloadRecord.requestId);
            const vendorName = getParam(payloadRecord.vendorName);
            const clientName = getParam(payloadRecord.clientName);
            const city = getParam(payloadRecord.city);
            const serviceType = getParam(payloadRecord.serviceType);
            const vehicleAvailabilityLocation = getParam(payloadRecord.vehicleAvailabilityLocation);
            const vehicleAvailableTime = getParam(payloadRecord.vehicleAvailableTime);
            const ltpocName = getParam(payloadRecord.ltpocName);
            const ltpocPhone = getParam(payloadRecord.ltpocPhone);
            const ltpocEmail = getParam(payloadRecord.ltpocEmail);
            // Validation
            if (!vehicleId) {
            ...
            const sentNotifications = (requestData?.notificationsSentToFO || []);
          ...
          `);
            ...
            const requestLtpoc = Array.isArray(requestData?.ltpocDetails)
                ? requestData.ltpocDetails.find((entry) => entry?.vehicleNumber === vehicleId)
                : null;
      ...
      `;
        ...
        `;
            ...
            const resolvedVehicleAvailabilityLocation = safeValue(vehicleAvailabilityLocation) || requestVehicleAvailabilityLocation;
        ...
        `;
            ...
            const resolvedVehicleAvailableTime = safeValue(vehicleAvailableTime) || requestVehicleAvailableTime;
      ...
      `;
        ...
        `,
            ...
            await requestRef.update({
                notificationsSentToFO: firebase_admin_1.default.firestore.FieldValue.arrayUnion(vehicleId),
