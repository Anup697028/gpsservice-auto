...
type VendorNotificationPayload = {
  ...
  driverDetails?: Array<{ vehicleNumber: string; driverName: string; driverNumber: string }>;
};
    ...
    try {
      ...
      const vehicleRows = (vehicles ?? [])
        .map((vehicle: { vehicleNumber: string; isNewTrip?: boolean }) => 
          `<tr><td style="padding: 8px; border: 1px solid #ddd;">${vehicle.vehicleNumber}</td><td style="padding: 8px; border: 1px solid #ddd;">${vehicle.isNewTrip ? 'New Trip' : 'Registered'}</td></tr>`)
        .join('');

      const driverRows = (driverDetails ?? [])
        .map((driver: { vehicleNumber: string; driverName: string; driverNumber: string }) => 
          `<tr><td style="padding: 8px; border: 1px solid #ddd;">${driver.vehicleNumber}</td><td style="padding: 8px; border: 1px solid #ddd;">${driver.driverName}</td><td style="padding: 8px; border: 1px solid #ddd;">${driver.driverNumber}</td></tr>`)
        .join('');
      ...
      await transporter.sendMail({
        ...
        text: [
          ...
          ``,
          'DRIVER DETAILS:',
          (driverDetails ?? []).map((d: { vehicleNumber: string; driverName: string; driverNumber: string }) => 
            `${d.vehicleNumber} - ${d.driverName} - ${d.driverNumber}`).join('\n'),
        ].join('\n'),
      });
